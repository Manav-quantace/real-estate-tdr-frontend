// app/(protected)/subsidized/dashboard/page.tsx
...
async function getProjects(baseUrl: string, cookie: string) {
    const res = await fetch(`${baseUrl}/api/projects?workflow=subsidized`, {
        cache: 'no-store',
        headers: { cookie },
    })
...
async function getValuer(baseUrl: string, cookie: string, projectId: string) {
    const res = await fetch(
        `${baseUrl}/api/subsidized/valuer?workflow=subsidized&projectId=${projectId}`,
...
async function getCurrentRound(baseUrl: string, cookie: string, projectId: string) {
    const res = await fetch(
        `${baseUrl}/api/rounds/current?workflow=subsidized&projectId=${projectId}`,
        { cache: 'no-store', headers: { cookie } }
    )
    if (!res.ok) return null
    return res.json()
...
export default async function SubsidizedDashboardPage() {
    ...
    const items = await Promise.all(
        projects.map(async (proj: any) => {
            ...
            return {
                ...
                title: proj.title ?? '',
                category: proj.category ?? 'subsidized',
