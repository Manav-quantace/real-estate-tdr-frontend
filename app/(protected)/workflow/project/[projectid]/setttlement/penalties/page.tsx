...
async function getPenaltyEvents(workflow: string, projectId: string) {
    const res = await fetch(
        `${process.env.API_URL}/${workflow}/${projectId}/settlement/penalties`,
        { cache: "no-store" }
    );

    if (!res.ok) {
        throw new Error("Failed to fetch penalty events");
    }

    return res.json();
}

export default async function PenaltyEventsPage({ params }: Props) {
    const data = await getPenaltyEvents(params.workflow, params.projectId);
    ...
    return (
        <div className="rounded-2xl border p-6">
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h1 className="text-xl font-semibold">
                        Penalty Events — Confiscation Rendering
                    </h1>
            ...
            <div className="mt-6 grid gap-3">
                {data.penalty_events?.length ? (
                    data.penalty_events.map((e: any, idx: number) => (
                    ...
                    <div className="rounded-2xl border p-4 text-sm text-slate-600">
                        No penalty events returned by API.
                    </div>
                )}
            </div>
        </div>
