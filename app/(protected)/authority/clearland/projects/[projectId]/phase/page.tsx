//app/authority/clearland/projects/[projectId]/phase/page.tsx
'use client'
...
export default function ClearlandPhasePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const membersRes = await fetch(`/api/clearland/membership/list?projectId=${projectId}`)
                const roundRes = await fetch(`/api/rounds/current?workflow=clearland&projectId=${projectId}`)
    ...
    const handleEnrollMember = async (participantId: string, role: string) => {
        try {
            const res = await fetch(`/api/clearland/membership/enroll`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            // ✅ Success toast
            setToast({
                message: `Participant enrolled as ${role}`,
                type: 'success',
            })

            // ✅ Refresh enrolled members (single source of truth)
            const membersRes = await fetch(
                `/api/clearland/membership/list?projectId=${projectId}`
            )
            if (membersRes.ok) {
    ...
    const handleRemoveMember = async (participantId: string) => {
        try {
            const res = await fetch(`/api/clearland/membership/remove`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
