...
const CLEARLAND_PHASES = ['INIT', 'DEVELOPER_ASK_OPEN', 'BUYER_BIDDING_OPEN', 'PREFERENCES_COLLECTED', 'LOCKED', 'COMPLETED']

export function PhaseTransitionCard({ currentPhase, onTransition, isLoading }: PhaseTransitionCardProps) {
    const [selectedPhase, setSelectedPhase] = useState('')
    const [showConfirm, setShowConfirm] = useState(false)
    const [transitioning, setTransitioning] = useState(false)

    const availablePhases = CLEARLAND_PHASES.filter((p) => {
        const currentIdx = CLEARLAND_PHASES.indexOf(currentPhase)
        const nextIdx = CLEARLAND_PHASES.indexOf(p)
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className='p-6 rounded-xl border border-primary/20 bg-card/50'>
            <h3 className='text-lg font-bold mb-4 text-foreground'>Phase Control</h3>

            <div className='mb-4'>
                <p className='text-sm text-foreground/60 mb-2'>Current Phase</p>
            ...
            <div className='mb-6 space-y-3'>
                <label className='text-sm font-medium text-foreground/80 block'>Transition to Phase</label>
                <select
                    value={selectedPhase}
            ...
            {showConfirm && (
                ...
                >
                    ...
                    >
                        <h4 className='text-lg font-bold mb-3 text-foreground'>Confirm phase transition</h4>
                        <p className='text-foreground/70 mb-6 text-sm leading-relaxed'>
                            Transition project to <span className='font-semibold'>{selectedPhase}</span> — this action will close current phase and append new phase entry to the timeline. Are you sure?
                        </p>
