...
export function PhaseTimeline({ entries, onExportCSV }: PhaseTimelineProps) {
    ...
    return (
        <div className='space-y-6'>
            ...
            >
                {entries.length === 0 ? (
                    <div className='py-8 text-center text-foreground/60'>No phase history available</div>
                ) : (
                    <>
                        <div className='absolute left-6 top-8 bottom-0 w-0.5 bg-linear-to-b from-primary to-transparent' />
                        {entries.map((entry, idx) => (
                            <motion.div key={idx} variants={itemVariants} className='pl-20 relative'>
                                <div
                                    className={`absolute left-0 w-14 h-14 rounded-full flex items-center justify-center border-2 ${idx === 0
                                ...
                                <div className='p-4 rounded-lg border border-primary/20 bg-card/50 hover:bg-card/80 transition-all'>
                                    <div className='flex items-start justify-between mb-2'>
                                        <span className='text-lg font-semibold text-primary'>{entry.phase}</span>
                                        <span className='text-xs text-foreground/60'>{entry.createdBy}</span>
                                    </div>
                                    <div className='text-sm text-foreground/70 mb-2'>
                                        Effective: {new Date(entry.effectiveFrom).toLocaleDateString()}
                                    </div>
                                    {entry.notes && <div className='text-sm text-foreground/60 italic'>{entry.notes}</div>}
                                </div>
                            </motion.div>
                        ))}
                    </>
